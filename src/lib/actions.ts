...
export async function signInAction(formData: FormData): Promise<void> { try { const supabase = await createClient(); const { error } = await supabase.auth.signInWithPassword({ email: String(formData.get("email") || ""), password: String(formData.get("password") || "") }); if (error) redirect("/login?error=1"); redirect("/dashboard"); } catch { redirect("/login?error=1"); } }
