...
export async function createUserAction(formData: FormData): Promise<void> { const parsed = createUserSchema.parse({ full_name: formData.get("full_name"), email: formData.get("email"), role_key: formData.get("role_key") }); const { supabase, profile } = await requireProfile(); await supabase.from("users_profiles").insert({ id: crypto.randomUUID(), organization_id: profile.organization_id, full_name: parsed.full_name, email: parsed.email, role_key: parsed.role_key }); await logAction("user.create", { email: parsed.email, role: parsed.role_key }); }
